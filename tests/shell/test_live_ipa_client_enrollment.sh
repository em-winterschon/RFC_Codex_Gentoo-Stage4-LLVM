...
assert_file_contains "${netboot_manifest}" 'reboot_durable_aaa_gate: pending'
assert_file_contains "${netboot_manifest}" 'without SSSD or libsss_ipa.so'
assert_file_contains "${identity_doc}" 'not yet reboot-durable'
