...
test_dry_run_emits_offline_openrc_network_materialization() {
  ...
  assert_contains "${output}" 'candidate_type="$(blkid -o value -s TYPE "${candidate}"'
  assert_contains "${output}" 'root_part_num="$(partx -g -o NR "${root_part}"'
  assert_contains "${output}" 'parted -s "${disk}" resizepart "${root_part_num}" 100%'
  assert_contains "${output}" 'e2fsck -fy "${root_part}"'
  assert_contains "${output}" 'resize2fs "${root_part}"'
  assert_contains "${output}" 'zpool import -f -N -R "${mnt}" "${zfs_pool_id}" "${zfs_import_name}"'
  assert_contains "${output}" 'zfs mount "${zfs_root_dataset}"'
