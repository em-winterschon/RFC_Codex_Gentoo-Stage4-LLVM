...
assert_file_contains "${PROFILE}" 'boot_storage_policy:'
assert_file_contains "${PROFILE}" 'idsdm_efi_boot: true'
assert_file_contains "${PROFILE}" 'sata_os_mirror_required: true'
assert_file_contains "${PROFILE}" 'sas_bays_reserved_for_zfs: true'
...
assert_file_contains "${METADATA}" 'IDSDM'
assert_file_contains "${METADATA}" 'SATA OS mirror'
...
assert_file_contains "${FMT2_DOC}" 'IDSDM EFI handoff'
assert_file_contains "${FMT2_DOC}" '/root/operator-private/fmt2/r630/r630-provisioning-forge-inspection.kvm-sfo200-sec-9923.tar'
assert_file_contains "${FMT2_DOC}" 'part /boot     --asprimary --fstype="ext4"'
assert_file_contains "${FMT2_DOC}" 'part /boot/efi --asprimary --fstype="efi"'
...
assert_file_contains "${FMT2_DOC}" 'iDRAC firmware: `2.86.86.86`'
assert_file_contains "${FMT2_DOC}" 'firmware-maintenance OS on 2026-05-20'
assert_file_contains "${FMT2_DOC}" 'did not enumerate a Mellanox/ConnectX'
assert_file_contains "${FMT2_DOC}" 'GlobalSlotDriverDisable=Disabled'
assert_file_contains "${FMT2_DOC}" 'reported no LLDP neighbors'
assert_file_contains "${FMT2_DOC}" '`Po613`'
...
assert_file_contains "${RDMA_DOC}" '2026-05-20 admission blocker'
assert_file_contains "${RDMA_DOC}" 'BIOS slot-disablement reports Slot 1 enabled'
assert_file_contains "${FMT2_INVENTORY}" 'Live 2026-05-20 state is a disposable Rocky 8.10 firmware-maintenance OS'
assert_file_contains "${FMT2_INVENTORY}" 'Arista Et6/1+Et6/3 still show 50G link on Po613'
