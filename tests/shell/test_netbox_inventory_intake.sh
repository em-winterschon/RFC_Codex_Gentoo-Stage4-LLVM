...
grep -Fq 'dcim/interfaces:admin_sun99_forge_099070:eth0' "${apply_plan_json}" || fail "apply plan did not include M70 Forge automation-admin interface"
...
if command -v ansible-playbook > /dev/null 2>&1; then
  ...
  trap 'rm -rf "${temp_dir}"; rm -f "${tmp_inventory}"' EXIT
