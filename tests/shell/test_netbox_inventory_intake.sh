...
grep -Fq 'dcim/devices:m70_canary' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include M70 canary"
...
grep -Fq 'dcim/interfaces:m70_canary:netboot0' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include M70 canary netboot0"
grep -Fq 'dcim/interfaces:m70_canary:eno4' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include M70 canary eno4"
...
grep -Fq 'dcim/power-ports:m70_canary:power0' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include M70 canary power port"
...
grep -Fq 'dcim/power-outlets:pdu_rfc99_corectrl_ap7901:outlet7' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include AP7901 outlet 7"
...
grep -Fq 'dcim/cables:pdu_rfc99_corectrl_ap7901:outlet7->m70_canary:power0' /tmp/netbox-intake-apply-plan.json || fail "apply plan did not include M70 canary PDU cable"
