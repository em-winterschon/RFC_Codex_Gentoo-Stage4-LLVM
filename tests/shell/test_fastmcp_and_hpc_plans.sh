...
grep -q "MCP-005" "${roadmap}"
grep -q "FMT2-005" "${roadmap}"
grep -q "RDMA-003" "${roadmap}"
grep -q "STOR-002" "${roadmap}"
grep -q "HPC-002" "${roadmap}"
grep -q "HPC-005" "${roadmap}"
...
grep -q "test_fastmcp_infra_servers.sh" "${REPO_ROOT}/tests/shell/run-tests.sh"
