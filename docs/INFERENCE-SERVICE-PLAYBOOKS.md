...
Use Noctosultris as the first x86_64 multi-GPU NVIDIA target only after host
management is live and source-of-truth records are applied:
...
The expected hardware target is 4x NVIDIA A4000, but the host OS, driver,
container runtime, and GPU state were not live-validated in this audit because
`172.16.99.165` was unreachable and live NetBox/DNS records were absent.
