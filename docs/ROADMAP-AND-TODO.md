...
| `PNR-012` | active | Replace failed OPNsense path with RouterOS after NetBox staging | `PNR-011` | CRS309 WIP remains the source config reference, but preferred target `gw_rfc99_mkccr2004_16g` is serial-discovered on `/dev/ttyUSB2` as RouterOS `7.19.6` / board `CCR2004-16G-2S+`. Next gate is a management-only bootstrap render/apply, backup/export, NetBox dry-run/apply, and validation before gateway cutover. |
