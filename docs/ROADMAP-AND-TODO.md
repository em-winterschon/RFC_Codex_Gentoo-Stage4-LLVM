...
| `FMT2-007` | active | Validate R630 front-end and RDMA fabric policy on Arista 7060 | `FMT2-002`, `FMT2-006`, `RDMA-001` | R630 target split is X710 `eno1`/`eno2` host-management LACP, X710 `eno3`/`eno4` VM front-end LACP/OVS/SR-IOV, and ConnectX-4 `enp130s0f0np0`/`enp130s0f1np1` 2x50GbE RoCEv2. 2026-05-19 evidence shows RDMA links active on `sec`/`ter`, but SR-IOV exposure is inconsistent and `sec` reports placeholder ConnectX MACs. Arista eAPI is reachable at `172.18.20.10:443`; SSH is filtered. |
