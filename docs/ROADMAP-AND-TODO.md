...
| `RDMA-003` | active | Deploy OFED driver path for BlueField, ConnectX-4, and newer ConnectX hosts | `RDMA-001`, `RDMA-002`, host PCI inventory | Issue #111. `nvidia_doca_ofed` now has read-only `lspci -Dnn` detection for NVIDIA/Mellanox `15b3` devices, BlueField-2 and ConnectX-4/5/6/7 markers, required mlx5/RDMA module intent, `ofed_info -s` validation, RoCE validation command planning, and an apply-time detection gate. `docs/workflows/rdma-fabric-promotion-readiness.yml` keeps live driver installation gated until selected package source, kernel/header compatibility, `ofed_info -s`, userspace/perftest tooling, and first gated apply evidence are captured. |
