...
| `MEM-003` | scaffolded | Implement Forge cross-machine continuity bootstrap | `MEM-001`, `MEM-002`, `MCP-004` | Issue #115. `scripts/forge_memory_spool.py` writes append-only local events, unsigned closeout manifests, local session summaries, `bootstrap-session` events, and filesystem-backed `publish-session` object-store artifacts with checksum metadata and overwrite protection. Remaining work is direct S3-compatible transport, manifest signing, and MCP facade methods. |
