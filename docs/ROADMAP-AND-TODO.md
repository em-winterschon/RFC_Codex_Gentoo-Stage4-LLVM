...
| `MEM-003` | scaffolded | Implement Forge cross-machine continuity bootstrap | `MEM-001`, `MEM-002`, `MCP-004` | Issue #115. Initial `scripts/forge_memory_spool.py` writes append-only local events and unsigned closeout manifests with secret-key rejection. Remaining work is object-store upload, manifest signing, MCP facade methods, and bootstrap reconciliation against repo/GitHub state. |
