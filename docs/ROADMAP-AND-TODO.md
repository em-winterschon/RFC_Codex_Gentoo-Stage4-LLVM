...
| `PNR-025` | active | Track Hasslehoff GPU compute and passthrough host policy | workstation VM profile | Hasslehoff is in `gpu_compute`; the `gpu_host_policy` role renders `nouveau` and `nvidiafb` blacklist controls plus a GRUB drop-in for GPU compute/passthrough hosts. K1200 and QLogic QL41232HOCU post-maintenance PCIe facts are recorded in host vars. |
