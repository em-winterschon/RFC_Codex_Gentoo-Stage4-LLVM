...
| `PNR-017` | active | Rebuild CRS309 as RouterOS 10GbE spine aggregation switch | `PNR-012` | CRS309 is live on RouterOS package and RouterBOARD firmware `7.22.2`; port conflict normalized by assigning CRS354 LACP to CRS309 `sfp-sfpplus2/3`, Hasslehoff CCR2004-1G-2XS-PCIe DACs to `sfp-sfpplus4/5`, CSS326 to `sfp-sfpplus8`, and CCR2004 gateway uplink to `sfp-sfpplus1`. Remaining work is cabling/validating CSS326 `sfp1` to CRS309 `sfp-sfpplus8` and the Hasslehoff CCR-PCI DAC pair. |
...
| `PNR-020` | active | Convert RouterOS spine/distribution changes into idempotent Ansible roles | `PNR-017`, `PNR-018` | Render-only role and playbook now emit CRS309/CRS354 RSC plus manifest for LACP and stale-state cleanup. Remaining work is read-only state collection, fixture-backed RouterOS render tests, and explicit serial-gated apply handling. |
| `PNR-021` | pending | Apply latest local fabric intake to live NetBox | `PNR-019`, `PNR-020` | Push updated CRS309, CRS354, CSS326, optic, LACP, and management facts into NetBox after role/test coverage is in place. |
