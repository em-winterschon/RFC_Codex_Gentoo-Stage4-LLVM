...
| `MEM-003` | scaffolded | Implement Forge cross-machine continuity bootstrap | `MEM-001`, `MEM-002`, `MCP-004` | Issue #115. `scripts/forge_memory_spool.py` writes append-only local events, unsigned closeout manifests, local session summaries, and `bootstrap-session` events that reconcile repo state, recent docs, recent memory sessions, issue state, and blocker state with secret-key rejection. Remaining work is object-store upload, manifest signing, and MCP facade methods. |
