...
- next dependency is moving Path B routing/IPAM source-of-truth into
  Proxmox/NetBox/CCR2004. The `ntfy` path no longer requires live Docker Hub
  pulls when a verified local OCI archive is staged.
...
| `ST-014` | completed | Replace `ntfy` upstream-image dependency | `ST-013` | `ntfy` now uses `build_mode: controlled-archive-preload`, local image `localhost/rfc1918/ntfy:v2.14.0`, `pull_policy: never`, and a required OCI archive plus SHA256 preload path under `/var/lib/container-services/preload`. |
...
| `ST-017` | active | Add sunrise critical-path validation gate | `ST-016`, `ADM-001`, `PNR-034`, `FMT2-002`, `HPC-002` | `scripts/validate-sunrise-critical-path.sh` provides a read-only daily launch gate for repo artifacts, M70/FMT2 OpenVPN transport, Hasslehoff backup readiness, local ntfy health, and SLURM/X12AGAIN prerequisites. See `docs/SUNRISE-CRITICAL-PATH.md`. |
| `ST-018` | active | Deploy RFC1918 CA trust and service TLS automation | `ST-016`, `AAA-009`, `DNS-003` | Non-secret service TLS matrix, gated CA trust role, gated service leaf role, RouterOS internal-CA import mode, and `scripts/validate-rfc1918-service-tls.sh` are in place. Live apply requires vaulted CA/leaf material and explicit `rfc1918_*_apply` gates. |
...
| `ADM-004` | planned | Promote X11SCL-IF as long-term Forge automation-admin platform | replacement heatsink arrival, physical install, inventory discovery | X11SCL-IF is preferred over the M70 for the long-term Forge control plane because it has Supermicro IPMI/OOB, Xeon E-2176G performance, native M.2 PCIe storage, Samsung 883-DCT mirrored SATA capacity, and PCIe expansion for QLogic RDMA NICs. Keep M70 as rollback until X11SCL-IF passes SSH, FreeIPA/SSSD, vault, GitHub CLI, NetBox, ntfy, CCR2004, Hasslehoff, X12AGAIN SoL, ZFS, and off-host backup gates. See `docs/X11SCL-IF-FORGE-AUTOMATION-ADMIN.md` and `docs/runbooks/x11scl-if-forge-admin-build.md`. |
...
| `PNR-030` | completed | Promote K10 and AP7901 PDU operational discoveries into NetBox intake | `PNR-010`, `WS-006`, PDU vault import | Repo-safe intake now covers K10 device/interface/IPAM/switchport metadata and AP7901 non-secret inventory/outlet mapping. PR `#103` carries the stacked first-enrollment intake. |
| `PNR-031` | completed | Add NetBox power-chain modeling for PDU outlet to host relationships | `PNR-030`, power-device model support confirmed | NetBox now has real DCIM cable objects for AP7901 `outlet6 -> gmktek_nucbox_k10_stage5_candidate:power0` and AP7901 `outlet4 -> admin_sun99_forge_099070:power0`; outlet 4 is labeled `admin-sun99-forge` in the AP7901 control panel. Live apply was snapshot-backed and idempotence validated with `created=0`, `updated=0`, `existing=303`. |
...
| `FMT2-002` | active | Validate FMT2 transport path from RFC99/SUN99 | BigNetwork SDN or compatibility OpenVPN VM | Temporary transport is live through M70 `openvpn.fmt2` at `172.16.99.70`, with CCR2004 static routes for `10.100.99.0/24`, `10.200.99.0/24`, and `172.18.20.0/24`. BigNetwork route propagation succeeded but Edge Lite forwarding/return behavior remains blocked; keep BigNetwork as the preferred steady-state path or replace the legacy SFO200 router path with managed RouterOS. See `docs/FMT2-OPENVPN-COMPAT-TRANSPORT.md`. |
...
| `FMT2-006` | planned | Stage Dell R630 HCI rebuild and preserve `ter` ZFS storage | `FMT2-002`, `PNR-024`, stable netboot artifacts | `docs/FMT2-R630-HCI-STAGED-REBUILD.md` defines `pri` as the first destructive rebuild candidate, `sec` as the second, and `ter` as the storage-preserving anchor. R630 installs must preserve the IDSDM EFI handoff to the SATA OS mirror and must not select SAS hot-swap bays as OS targets. `ter` must use existing pool `dstore` for VM images, zvols, backups, staging, and ISO/provisioning caches; its OS RAID1 SATA SSDs are OS-only. |
| `FMT2-007` | active | Validate R630 front-end and RDMA fabric policy on Arista 7060 | `FMT2-002`, `FMT2-006`, `RDMA-001`, `RDMA-003` | Issue #130. R630 target split is X710 `eno1`/`eno2` host-management LACP, X710 `eno3`/`eno4` VM front-end LACP/OVS/SR-IOV, and ConnectX-4 `enp130s0f0np0`/`enp130s0f1np1` 2x50GbE RoCEv2. 2026-05-19 evidence shows RDMA links active on `sec`/`ter`, but SR-IOV exposure is inconsistent, `ofed_info` is absent, in-kernel `mlx5` is active, and `sec` reports placeholder ConnectX MACs. Arista eAPI is reachable at `172.18.20.10:443`; SSH works from M70 through CheckMK via alias `7060` because `mgmt-acl` permits `10.200.99.27`. `docs/workflows/rdma-fabric-promotion-readiness.yml` centralizes production-admission blockers and keeps this issue live-gated. |
...
| `MEM-001` | scaffolded | Design distributed Codex shared memory service | object storage backend, MCP control plane | Target backend is S3-compatible object storage with append-only event logs, signed manifests, replayable summaries, and per-agent namespace isolation. `docs/FORGE-MEMORY-OBJECT-STORE.md` now defines the initial storage contract consumed by the Forge memory spooler and MCP facade. |
| `MEM-002` | scaffolded | Add S3 object-store service role for agent memory artifacts | storage fabric and private CA | `forge_memory_object_store` renders the endpoint env, append-only bucket policy intent, lifecycle retention, object naming, server-side encryption, object-lock/versioning requirements, and audit-log prefix without storing S3 access keys or secret keys in profile data. |
| `MEM-003` | completed | Implement Forge cross-machine continuity bootstrap | `MEM-001`, `MEM-002`, `MCP-004` | Issue #115. `scripts/forge_memory_spool.py` writes append-only local events, signed or unsigned closeout manifests, local session summaries, `bootstrap-session` events, and filesystem-backed or S3-compatible `publish-session` object-store artifacts with checksum metadata. `scripts/mcp_servers/forge_memory_mcp.py` wraps read methods and gated `memory.append_event` with audit artifacts. Future hardening belongs in MEM-001/MEM-002/MCP-004 follow-ups. |
...
| ID | Status | Task | Depends On | Notes |
| --- | --- | --- | --- | --- |
| `RDMA-001` | planned | Define RoCE-v2 switch tuning profiles | CRS309/CRS354/Arista leaf-spine model, NIC inventory | Track PFC/ECN/DSCP, MTU, queueing, LAG behavior, lossless class boundaries, and rollback for mixed storage/VM traffic. FMT2 R630/Arista 7060 policy now prefers independent ConnectX-4 50GbE RDMA paths before any LACP or OVS switchdev promotion. |
| `RDMA-002` | completed-pending-closeout | Define host RDMA storage client baseline | `RDMA-001`, NFS/NVMe-oF/iSER requirements | Issue #117. Current `nfs-storage-client` role/profile covers NFSv3 TCP, NFSv4.2 identity, NFS-RDMA intent, and multipath package/kernel policy. `docs/workflows/rdma-fabric-promotion-readiness.yml` marks the baseline defined and moves live protocol promotion to the shared #111/#130 production gates. |
| `RDMA-003` | active | Deploy OFED driver path for BlueField, ConnectX-4, and newer ConnectX hosts | `RDMA-001`, `RDMA-002`, host PCI inventory | Issue #111. `docs/workflows/doca-host-source-of-truth.yml` defines two isolated lanes: DOCA 2.9.4 LTS for ConnectX-4 on Rocky 9.6 kernel `5.14.0-570.12.1.el9_6.x86_64`, and DOCA Host 3.3.0 for BlueField-2/ConnectX-5 on Rocky 10 kernel `6.12.0-124.8.1.el10_1.x86_64`. Build on `ter` inside a container, systemd-nspawn root, or VM; publish kernel, DOCA, OFED, firmware evidence, DKMS logs, and SHA256SUMS into separate NASA repos under `/opt/storage/nfs/nasa/nasa-yum-repo-doca-host/`. Live install remains blocked until `ofed_info -s`, userspace/perftest tooling, and first gated apply evidence are captured. |
...
| `HPC-002` | active | Build SLURM-first workload scheduler pilot | `AAA-009`, `DNS-003`, `E2ET-002`, `BUILD-VM-001`, observability baseline | Issue #116. Live VM pilot evidence exists for controller `sched-sun99-slurmctl-099071` and first worker `sched-sun99-slurmwkr-099072`. Next expansion should keep the controller off X12AGAIN and add a second physical M70 as `sched-sun99-slurmwkr-099073` after MAC, switch port, PDU outlet, DNS/IPAM, NetBox, MUNGE, `slurmd`, rsyslog, Prometheus, and `srun hostname` validation. Avoid nesting the full pilot inside the current Forge M70 except as a last-resort syntax test. |
| `HPC-003` | planned | Add scheduler node health and feature inventory | `HPC-002`, NetBox host inventory, `RDMA-002` | Issue #118. Generate SLURM node features from NetBox and Ansible inventory: CPU generation, ISA flags, memory class, GPU model, RDMA capability, storage locality, and power-control method. Integrate NHC, E2ET, and observability. |
...
| `WS-006` | active | Validate GMKtek K10 bare-metal Stage5 install before X12AGAIN | `WS-004`, K10 EFI handoff validation | K10 is on CSS326 `ge16`; PXE NIC slot `04:00:00`, MAC `84:47:09:5F:21:64`, static lease `172.16.99.156`. Native HTTPBoot stalled after DHCP, but generated PXE IPv4 with option 67 `k10-ipxe.efi`, `next-server=172.16.99.88`, OpenRC `netboot-tftp`, embedded iPXE, and `workstation-validation.ipxe` validates the live installer path. On 2026-05-18 the disk install completed far enough to create mirrored `rpool`, ZFSBootMenu fallback EFI, and a UEFI `ZFSBootMenu K10` entry, but first installed-disk boot failed in dracut because the generated ZFSBootMenu dataset command line omitted `root=ZFS=rpool/ROOT/gentoo`; PiKVM showed repeated `ZFS: Unable to import pool rpool` and `No sysroot.mount exists`. The boot-role fix now preserves `root=ZFS=...` and uses argv-safe `zfs set`; next action is recover K10 to netboot/live rescue or local console, apply the corrected boot role, and rerun the installed-disk boot gate. Do not treat `172.16.99.160` as K10 until the duplicate/unknown host state is resolved. |
