...
- `collectd` for legacy host/service metrics aggregation and Graphite-format forwarding
...
Prometheus now remote-writes all scrape data to the VictoriaMetrics VM. Prometheus
keeps short operational retention for scrape/debug workflows, while VictoriaMetrics
is the canonical long-retention metrics store.

collectd agents can forward to the central collectd aggregator on the
VictoriaMetrics VM. That aggregator writes to VictoriaMetrics through the Graphite
listener and also exposes a `write_prometheus` endpoint for Prometheus scrape
visibility.

The VictoriaMetrics data path is modeled as `/srv/metrics/victoriametrics` and is
backed by a managed NFSv3 mount in the example profile. The NFS role already
models NFSv4, NFS-TCP, NFS-RDMA, and multipath intent so this can move to
RBAC-aware NFSv4 or RDMA-backed transport later without changing the service
topology.
...
- collectd aggregation scrape and Graphite ingest
...
`vars/service_telemetry_matrix.yml` tracks the expected telemetry attachment for
each infrastructure service role. It treats `node_exporter` and collectd as the
baseline for hosts and VMs, adds `podman_exporter` for container hosts, uses
blackbox probes for service readiness/SLO checks, and maps SNMP/IPMI/Redfish
collection to the relevant power, OOB, BMC, and hardware inventory domains.
...
5. Convert the live service-package bridge into a first-class post-install
   Ansible apply path so fresh Stage4 service VMs do not need manual Portage
   commands after provisioning.
6. Add CA trust handling for HTTPS ntfy notifications so local HTTPS notices do
