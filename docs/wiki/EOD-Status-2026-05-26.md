...
Rendered diagram assets are stored in the repository under `docs/diagrams/`.
The wiki publishing script currently mirrors Markdown files only, so the wiki
page keeps the PlantUML source inline and the rendered files remain repo
artifacts until wiki asset publishing is extended.
