...
| `WS-004` | active | Validate Hasslehoff GPU workstation VM | `WS-003`, `PNR-025`, `PNR-028` | VM `1094` is running at `172.16.99.94` with K1200 passthrough and serial console. NVIDIA R580 plus CUDA 12.9.1 are validated with `nvidia-smi` and `nvcc`; PiKVM Direct H.264 is validated through the managed `1280x720@60` NVIDIA Xorg display policy. Current package gate is the Intel/K10 GPU stack rerun: `intel-metrics-library` patch context was fixed and a guarded rerun is queued after the active `emerge`; `llvm-core` needs fresh-log review if it fails again. |
...
| `WS-006` | blocked | Validate GMKtek K10 bare-metal Stage5 install before X12AGAIN | `WS-004`, K10 firmware console access | K10 was moved to CSS326 `ge16`, but no confirmed DHCP/iPXE lease was observed. Next removal condition is local console/PiKVM BIOS access, enabling UEFI PXE/iPXE, and recording the real NIC MAC. Do not treat `172.16.99.160` as K10 until the duplicate/unknown host state is resolved. |
