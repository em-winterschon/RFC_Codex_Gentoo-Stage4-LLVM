...
| `PNR-012` | planned | Replace failed OPNsense path with RouterOS after NetBox staging | `PNR-011` | CRS309 WIP remains the source config reference, but the preferred target is now `gw_rfc99_mkccr2004_16g` (`CCR2004-16G-2S+PC`). Execute only after serial discovery, backup/export, management-only bootstrap, NetBox dry-run/apply, and validation gates. |
