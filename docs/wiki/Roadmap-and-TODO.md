...
| `FMT2-005` | planned | Bring up BigNetwork L2 path for FMT2/SFO-200 discovery | `FMT2-002`, BigNetwork portal/device access | Issue #114. Tomorrow action item: finish the BigNetwork link so SUN99/RFC99 can reach FMT2 management networks and 100GbE-connected hosts. Once reachable, run connectivity discovery, promote verified records into NetBox, and feed Check_MK/observability onboarding. |
...
| `MEM-003` | planned | Implement Forge cross-machine continuity bootstrap | `MEM-001`, `MEM-002`, `MCP-004` | Issue #115. Bootstrap new Forge runtimes by reading repo state, latest memory manifest, latest EOD/SITREP, unresolved blockers, GitHub issues/project state, then appending a session-start event. Use local write-ahead spool when object storage is unavailable. |
