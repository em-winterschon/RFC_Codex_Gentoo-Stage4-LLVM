...
| `WS-004` | active | Validate Hasslehoff GPU workstation VM | `WS-003`, `PNR-025`, `PNR-028` | VM `1094` is running at `172.16.99.94` with K1200 passthrough and serial console. NVIDIA R580 plus CUDA 12.9.1 are validated with `nvidia-smi` and `nvcc`; PiKVM Direct H.264 is validated through the managed `1280x720@60` NVIDIA Xorg display policy. Intel workstation support is now display-only by default, so the base package path excludes IGC, Level Zero, `intel-compute-runtime`, `intel-metrics-library`, and `gmmlib` to avoid the forced `llvm:16` dependency. |
