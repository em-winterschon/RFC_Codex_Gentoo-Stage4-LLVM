...
| `ST-019` | planned | Add Redis-backed Ansible fact cache profile | Redis service endpoint, Ansible controller package policy | Use Redis for volatile fact caching to speed repeated playbook execution without disk persistence. Do not enable globally until Redis reachability and required Python client packages are validated on every automation controller; otherwise a missing cache service can break unrelated playbook runs. |
...
| `STOR-002` | active | Import coherent storage ADR reference set | `STOR-001`, `MEM-001`, `LLM-004` | Issue #112. Initial Project Coherent Flash scale model is documented as a simulation-only SLURM bundle in `docs/PROJECT-COHERENT-FLASH-SLURM-SCALE-MODEL.md`, mapping ADR-001 through ADR-009 to job classes, metrics, and conformance artifacts. |
...
| `WS-009` | active | Reimage X12AGAIN as LOX workstation plus hypervisor host | `WS-006`, `AAA-009`, backup completion | Target profile is now `metal-x12again-workstation-xen-coherent`, composing workstation Xorg/NsCDE, Xen/QEMU/libvirt, AAA, NFS/RDMA storage client intent, SLURM worker admission gates, Optane NVDIMM, BlueField2/RDMA, and Project Coherent Flash scale model policy. The no-mutation preflight is tracked in `docs/X12AGAIN-REIMAGE-PREFLIGHT.md` and `playbooks/x12again-reimage-preflight.yml`; X12AGAIN remains out of active SLURM workers until E2ET passes. |
