...
| `AAA-009` | pending | Persist K10 Stage5 installed-system SSSD policy | `AAA-007`, `WS-006` | Package-layer rootfs rebuild is complete; the remaining work is persistent hostname/FQDN behavior, secure first-boot FreeIPA OTP bundle delivery or disk-install enrollment, SSSD offline cache, sudo rule refresh, local break-glass account access, and reboot durability before enrolling additional Linux clients. |
