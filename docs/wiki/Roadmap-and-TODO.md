...
| `PNR-034` | active | Prepare Hasslehoff for scheduled external backups | `PNR-016`, external target selection, backup storage validation | Issue #123. Thursday 2026-05-14 track. `scripts/backup-hasslehoff-scheduled.sh` now wraps the config capture with lockfile protection, manifest writing, optional rsync mirror, checksum verification, and ntfy reporting. Remaining work is selecting the durable external target, adding selected VM/LXC coverage, restore validation, and explicit retention. Raw backup artifacts stay out of git. |
