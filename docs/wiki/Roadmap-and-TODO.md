...
| `PNR-034` | active | Prepare Hasslehoff for scheduled external backups | `PNR-016`, external target selection, backup storage validation | Issue #123. Thursday 2026-05-14 track. Promote the manual config capture into scheduled external coverage with target reachability independent of X12AGAIN, selected VM/LXC backups, optional ZFS send or encrypted borg/restic, restore validation, ntfy/observability reporting, and explicit retention. Raw backup artifacts stay out of git. |
...
| `HPC-002` | active | Build SLURM-first workload scheduler pilot | `AAA-009`, `DNS-003`, `E2ET-002`, `BUILD-VM-001`, observability baseline | Issue #116. Live pilot is online as of 2026-05-13: controller `sched-sun99-slurmctl-099071` (`172.16.99.71`) runs MUNGE, MariaDB, `slurmdbd`, and `slurmctld`; worker `sched-sun99-slurmwkr-099072` (`172.16.99.72`) runs MUNGE and `slurmd`; `sinfo`, `scontrol show nodes`, `srun --nodes=1 --ntasks=1 hostname`, and full shell validation pass. Remaining work is scheduler observability, NetBox feature generation, durable health checks, and adding X12AGAIN only after workstation+hypervisor+RDMA E2ET passes. |
