...
| `BUILD-VM-002` | active | Move SUN99 netboot publishing off X12AGAIN | `BUILD-VM-001`, `WS-006` | Desired netboot publisher is Hasslehoff VM `1088`, `boot-sun99-netboot-099088.rfc1918.host` / `172.16.99.88`, serving `/var/lib/netboot/path-b` over HTTP `8080` and TFTP. Repo desired state now points K10 manifests, netboot publisher group vars, and CCR2004 DHCP `next-server` at `172.16.99.88`; live cutover waits for VM provisioning, rsync/checksum of X12AGAIN `/var/lib/netboot/path-b`, TFTP/HTTP validation, and then gated RouterOS apply. |
