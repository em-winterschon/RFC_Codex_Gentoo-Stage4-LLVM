...
**Goal:** Bring the first RFC99 SLURM pilot online at `sched-sun99-slurmctl-099071.rfc1918.host` (`172.16.99.71`) with one validated first worker and observability gates.

**Architecture:** Deploy a small non-production SLURM controller VM with local MUNGE, MariaDB, `slurmdbd`, and `slurmctld`, then attach one disposable worker using the existing `slurm-worker-node` profile. X12AGAIN remains out of the critical path until it is reimaged and passes workstation+hypervisor+RDMA E2ET.
...
observability requirements, smoke tests, Project Coherent Flash gate, and
backout plan.
