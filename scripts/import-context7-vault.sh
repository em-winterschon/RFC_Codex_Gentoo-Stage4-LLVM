...
DEFAULT_TOKEN_FILE="/root/.codex/secrets/context7_api_key"
...
usage() {
  cat << EOF
Usage: import-context7-vault.sh [TOKEN_FILE] [VAULT_FILE]

Imports the operator-private Context7 API token into an encrypted Ansible Vault
file.
...
export CONTEXT7_TOKEN_FILE="${token_file}"
...
token_file = Path(os.environ["CONTEXT7_TOKEN_FILE"])
...
if not token:
    raise SystemExit("Context7 token file is empty")

payload["vault_context7_api_token"] = token
payload["vault_context7_token_name"] = "CONTEXT7_API_KEY"
payload["vault_context7_token_owner"] = "Forge"
payload["vault_context7_purpose"] = (
    "Authenticated Context7 MCP documentation retrieval for Codex operations"
)
